...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[GALLERY].[GalleryGetByCategoryXML]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [GALLERY].[GalleryGetByCategoryXML]
GO
...
BEGIN
	...
	IF(@P_TOTAL_COUNT = 0)
	BEGIN
		DECLARE @emptyValue XML='<Parent></Parent>'
		SELECT @emptyValue
	END
	ELSE
	BEGIN
